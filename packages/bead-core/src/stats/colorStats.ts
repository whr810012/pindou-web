...
export function computeColorStats(
  grid: MappedGrid,
  brand: BrandSystem,
  codeLookup: (paletteId: string, brand: BrandSystem) => string,
): ColorStat[] {
...
export function countTotalBeads(grid: MappedGrid): number {
  let total = 0
  for (const row of grid) {
    for (const cell of row) {
...
export function countCompleted(grid: MappedGrid, completedCells: Set<string>): number {
  let total = 0
  for (let row = 0; row < grid.length; row++) {
    for (let col = 0; col < (grid[0]?.length ?? 0); col++) {
      const cell = grid[row][col]
      if (!cell.isExternal && completedCells.has(`${row},${col}`)) total++
    }
  }
  return total
}
...
export function getConnectedRegions(grid: MappedGrid, paletteId: string): ConnectedRegion[] {
    ...
    for (let col = 0; col < width; col++) {
      ...
      while (stack.length > 0) {
        ...
        for (const [dr, dc] of neighbors) {
          ...
          if (next.isExternal || next.paletteId !== paletteId) continue
          visited[nr][nc] = true
          stack.push([nr, nc])
        }
      }
